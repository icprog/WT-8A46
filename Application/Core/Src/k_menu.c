...
/* External variables --------------------------------------------------------*/
extern GUI_CONST_STORAGE GUI_BITMAP bmLogoSDJT01;
extern GUI_CONST_STORAGE GUI_BITMAP bm_BackGround480x272;
...
WM_HWIN hWin_Title = 0;
WM_HWIN hWin_Statue = 0;
...
//    GUI_DrawHLine(ySize - 1, 0, xSize );
		...
		/* Draw logo */
		GUI_DrawBitmap(&bmbmLZYLogo40x20, 2, 2);
    ...
    GUI_DispStringAt("Ö÷²Ëµ¥", xSize - 80, 3);
    break;
...
//    }
  ...
  case WM_PAINT:
    ...
    /* Draw background */
		GUI_SetColor(0x808080);
//    GUI_DrawHLine(0, 0, xSize );
//		GUI_SetColor(0x404040);
//		GUI_DrawHLine(0, 1, xSize );
//    GUI_SetColor(0x40303030);
    GUI_FillRect(0, 0, xSize - 1, ySize - 1);
...
//	GUI_DrawBitmap(&bmbmLZYLogo40x20, 0, LCD_GetYSize()-25);
//}
...
void k_InitMenu(void) 
{
  ...
  hWin_Title = WM_CreateWindowAsChild(0,
                         0,
                         LCD_GetXSize(),
                         25,
                         WM_HBKWIN, 
	...
	hWin_Statue = WM_CreateWindowAsChild(0,
                         LCD_GetYSize()-25,
                         LCD_GetXSize(),
                         25,
                         WM_HBKWIN, 
...
//  }
  ...
  //hItem = BUTTON_CreateEx(0, LCD_GetYSize()-26, 40, 23, WM_HBKWIN, WM_CF_SHOW, 0, ID_BUTTON_BKGND);
  //WM_SetCallback(hItem, _cbButton);
	
//	module_prop[1].module->startup(WM_HBKWIN, 0, 25);
}
...
void wt_SetText_Title_Menu(const char * pInfo) 
{
	TEXT_Handle hText;
  ...
  if (WM_IsVisible(hWin_Title)) {
    hText = WM_GetDialogItem(hWin_Title, GUI_ID_TEXT1);
