...
//	int  										wheelval=0;
  ...
  case WM_INIT_DIALOG:
    ...
    //
	  ...
	  hItem = WM_GetDialogItem(pMsg->hWin, ID_LISTBOX_0);
		if(TestFolder.number_TotalFile <= 7)
		{
	...
	case MY_MESSAGE_DOWN:
		...
		if(WM_HasFocus(hItem))
		{
			if(TestFolder.number_TotalFile <= 7 && select_index < 7)
			{
				LISTBOX_IncSel(hItem);
				select_index = LISTBOX_GetSel(hItem);
			}
			else 
			{
				if(select_index >6 )
				{
					if(LISTBOX_GetSel(hItem)==6)
					{				
						...
						for(i=select_index;i<select_index+7;i++)
						{
							if(i<TestFolder.number_TotalFile)
							{
								LISTBOX_AddString(hItem, (char *)TestFolder.FilesName[i]);
					...
					{
						LISTBOX_IncSel(hItem);
						if(select_index < TestFolder.number_TotalFile -1 ) {select_index ++;}
					}
				}
				else
				{
			...
			else 
			{
				if(select_index >6 )
				{
					...
					{				
						...
						for(i=select_index-7;i<select_index;i++)
						{
							LISTBOX_AddString(hItem, (char *)TestFolder.FilesName[i]);
						}
				...
				else
				{
					...
					LISTBOX_SetSel(hItem,select_index);
					LISTBOX_DecSel(hItem);
	...
	case MY_MESSAGE_OK:
		...
		TestFolder.number_CurrentFile = select_index;
